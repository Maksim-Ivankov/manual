...
# 16 урок
less_16 = ft.Container(
    ft.Column(controls=[
        ft.Container(ft.Text('Подведем итоги и начнем учиться',text_align='center'),width=700),
        ft.Container(ft.Text('ТИПЫ ДАННЫХ',text_align='center'),width=700),
        ft.Container(ft.Text('Теперь, когда вы написали десяток игр и потренировались в написании кода, можно подробнее разобрать код. Что-то вы поняли интуитивно, что-то просто переписывали с учебника. Но пока (скорее всего) вы еще не можете писать самостоятельный код и разрабатывать игры. Речь идет не о постройках с тулбокса, а именно о создании любой игры, идея для которой может возникнуть в вашей голове.'),width=700),
        ft.Container(ft.Text('Первое, с чем разберемся - это типы данных. В роблоксе их 3 - число, строка и логический.'),width=700),
        ft.Container(ft.Text('Попробуйте сделать то, что вы уже ни раз делали. Запринтуем пару значений:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/1.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Сразу бросается в глаза, что разные типы данных отличаются по цвету и жирности. Строка - всегда зеленая, числа - желтые обычные, логические типы данных - желтые толстые.'),width=700),
        ft.Container(ft.Text('Со строками и числами все понятно, а логический тип данных - это true (истина) и false (ложь). Логический тип дынных принимает только 2 значения.'),width=700),
        ft.Container(ft.Text('ПОЛУЧЕНИЕ ИГРОВЫХ ОБЪЕКТОВ',text_align='center'),width=700),
        ft.Container(ft.Text('Игры в роблокс студио построены по принципу иерархии (как матрешка, одно лежит внутри другого). И вы наверняка тысячу раз уже писали конструкции типа game.Workspase....'),width=700),
        ft.Container(ft.Text('Все папки, что лежат в эксплорере и создают вашу игру. У большинства школьников сложно с английским, поэтому почти весь роблокс выглядит, как абра кадабра. Но что будет, если попробовать перевести все папки в експлорере?'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/2.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Половина папок все еще не особо понятно, зачем вообще нужна, но с другой половиной - проще. Понятно, что это. И вы часть уже использовали. Вы работали с Starter GUI - создавали здесь меню и выбор команды в кс. Всё, что связано с графическим интерфейсом. И теперь можно понять, что если, к примеру, нужно вывести индикатор жизней в правый верхний угол экрана, то делать мы это будем внутри Starter GUI.'),width=700),
        ft.Container(ft.Text('Так же мы работали с Teams - командами, когда делали урок Контер страйк и содавали выбор команд. ВСё логично, не правда ли?'),width=700),
        ft.Container(ft.Text('Давайте решим задачу - сделать бесплейт прозрачным. Гэе он находится? В папке рабочего пространства, которая в свою очередь находится в игре (game) Все папки рабочего пространства (workspace) находятся в игре. Соответственно, пишем:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/3.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('И, как всегда, чудесным образом работают подсказки. Они сильно облегчают жизнь разработчика, особенно, если не знаешь английский. '),width=700),
        ft.Container(ft.Text('Результат - бесплейт прозрачный. '),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/4.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Такая структура программы чаще всего называется деревом. Game - это корень, в нём лежит вообще все. Из корня выходит ствол - это все папки, расположенные внутри Explorer - Workspase, StarterGui,Team и прочие. Из каждой папки ствола выходят свои ветки, из Workspace выходят все объекты, что вы на него добавите.'),width=700),
        ft.Container(ft.Text('Так же можно менять любые свойства любого предмета, расположенного где угодно в игре. М это уже делали много раз. '),width=700),
        ft.Container(ft.Text('Задание - запрограммируйте материал бесплейта - кирпич.'),width=700),
        ft.Container(ft.Text('Должно получиться так:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/5.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('ФУНКЦИИ',text_align='center'),width=700),
        ft.Container(ft.Text('Следующее, что хотелось бы обсудить - это функции. Функции в скриптах используются довольно часто. Они делают код более читаемым, отлавливать ошибки коде с функциями довольно просто. Вы уже использовали функции, например когда делали игру с яблоками. Там была такая штука:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/6.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Давайте подробнее разберем, как работают функции.'),width=700),
        ft.Container(ft.Text('Еще на этапе игры со сбором яблок препод вам говорил (по крайней мере должен был рассказать на этапе, когда объяснял работу коду, если вы не супер крутой и вам не нужно была помощь😃'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/7.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('У каждой функции есть имя, которое вы придумываете сами. Можете написать вообще все, что угодно, но лучше бы имени отражать то, что делает функция. В примере имя функции - это h1'),width=700),
        ft.Container(ft.Text('Всё, что идет после до end - это тело функции. В нашем случае тело функции, это:'),width=700),
        ft.Container(ft.Text('n = n+1'),width=700),
        ft.Container(ft.Text('print(n)'),width=700),
        ft.Container(ft.Text('Тело функции - это то, что выполняет функция. '),width=700),
        ft.Container(ft.Text('Чтобы функция исполнилась, её нужно вызвать. Вызывается функция так:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/8.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Весь код выглядит так:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/9.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Запустим и посмотри, что произойдет:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/10.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Изначально, переменная n была равно 0. Как только мы вызвали функцию, исполнилось тело функции, то есть n=n+1, так как n была равна нулю, то получаем n = 0+1 или n=1, ну и следом мы распечатали результат print(n) и получили 1 в консоли снизу.'),width=700),
        ft.Container(ft.Text('Что будет если вызвать функцию несколько раз? Например, 3 раза? Что напишет консоль. Попробуй ответить на этот вопрос, а потом смотри ниже, правильно или нет.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/11.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Вызывая 3 раза подряд функцию, у нас 3 раза подряд исполнится тело функции. То есть фактически выполнится следующее:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/12.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('3 раза исполнили тело функции. '),width=700),
        ft.Container(ft.Text('Это мы еще раз разобрали то, что вы прошли в уроке с яблоками. Но функции умеют немного больше, чем в примере выше. '),width=700),
        ft.Container(ft.Text('В примере выше функция была самой простой, она на принимала никакие параметры и ничего не возвращала. Давайте же напишем более интересную функцию - функцию по нахождению гипотенузы по теореме Пифагора. Не знаете что это? Значит ты маленький и в школе еще не изучал прямоугольные треугольники. '),width=700),
        ft.Container(ft.Text('Выглядит теорема так:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/13.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Прямоугольный треугольник - это такой треугольник, у которого один из углов прямой, 90 градусов. И в таком треугольнике, зная две стороны, можно найти третью. Стороны, лежащие у прямого угла называются катетами, сторона, напротив прямого угла - гипотенузой. Если  сложно, то не ной и читай дальше. Всё равно в школе это будешь проходить скоро. Так вот квадрат длинны гипотенузы равен сумме квадратов катетов. '),width=700),
        ft.Container(ft.Text('Давай напишем функцию, которая будет по двум катетам находить гипотенузу. Делается это так:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/14.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Здесь произошло примерно следующее'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/15.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('В скобках имени функции, мы моем указывать какие параметры она принимает. Имена параметров могут быть любые. И количество их может быть любое.'),width=700),
        ft.Container(ft.Text('Далее в переменную c записывается результат вычисления гипотенузы.'),width=700),
        ft.Container(ft.Text('return - вернуть, то есть функция вернет значение гипотенузы.'),width=700),
        ft.Container(ft.Text('print(gipo(3,4)) - сначала выполнится вычисление функции, то есть корень квадратный из суммы квадратов 3 и 4, и функция вернет результат - 5. Этот результат и будет отработан print()'),width=700),
        ft.Container(ft.Text('Кстати, print() - это тоже функция. Внутри скобок мы передаем параметр функции print. '),width=700),
        ft.Container(ft.Text('Теперь ты знаешь чуть больше о функциях, если, конечно, всё понял.'),width=700),
        ft.Container(ft.Text('УСЛОВИЯ',text_align='center'),width=700),
        ft.Container(ft.Text('Теперь поговорим о конструкции условий. Условия - это if else. Если иначе. Это вопрос, на который можно ответить да или нет и выполнить какое-то действие, если да или нет. С условиями вы так же сталкивались ранее в игре по сбору яблок.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/16.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Ты писал что-то подобное в игре по сбору яблок. В функции, которая считала съеденные яблоки было условие - если количество съеденных яблок равно 5, то распечатай - победа. И еще там было - уничтожь килблок, но мы это тут писать не будем.'),width=700),
        ft.Container(ft.Text('Что необычного в условии? '),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/17.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Дословно читается так: если n равно 5, тогда.'),width=700),
        ft.Container(ft.Text('Здесь может смутить двойное равно. Что же это такое?'),width=700),
        ft.Container(ft.Text('n=5 - это операция присваивания. То есть мы в переменную n кладём число 5.'),width=700),
        ft.Container(ft.Text('n==5 - это операция сравнения. То есть мы спрашиваем - переменная n равна 5? На этот вопрос можно ответить да или нет. Так вот условие ответа да и лежит внутри тела if. Если ответ будет нет, то тело if не выполнится. Вот собственно и всё.'),width=700),
        ft.Container(ft.Text('Так же у условия есть блок иначе. Давайте напишем такую программу:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/18.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Мы добавили блок иначе. Получаем - если n равна 5, то распечатываем - победа, иначе распечатываем - ты пока не победил.'),width=700),
        ft.Container(ft.Text('Далее я вызвал функцию 6 раз. На пятый вызов переменная n была равна 5, и у нас сработало условие и выполнилось тело if на ответ да, а на шестой вызов переменная n стала равна 6 и мы снова не попали в условие. Забавно, не правда ли?'),width=700),
        ft.Container(ft.Text('СОБЫТИЯ',text_align='center'),width=700),
        ft.Container(ft.Text('С событиями вы уже сталкивались. Событие - это что-то, что ожидаемо может произойти, например - касание блока. Так, когда мы делали кил блок, мы прописывали условие - если касаемся блока, тогда. Так вот касание блока - это и было событие.'),width=700),
        ft.Container(ft.Text('Очень просто объяснить события на примере кнопок в графическом интерфейсе. Есть событие наведения на кнопку, есть событие нажатия на кнопку. '),width=700),
        ft.Container(ft.Text('Видели приколы в браузере, когда при наведении на кнопку, она как-то изменялась? Увеличивалась, или цвет меняла? Так вот это программисты позаботились о вас и добавили красоту - обработку события. Когда наводишь на кнопку, перекрась кнопку.'),width=700),
        ft.Container(ft.Text('В случае с кил блоком - когда персонаж касается плиты, убей его. '),width=700),
        ft.Container(ft.Text('В роблоксе есть два типа событий, которые используются чаще всего.'),width=700),
        ft.Container(ft.Text('Первое событие, которое изучим (или вспомним) - это когда игрок присоединяется к серверу. Оно называется событием добавления игрока и находится в папке Players'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/19.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('То, что мы напишем в скобках - и будет исполняться, когда новый игрок присоединяется к игре. Причем, это будет срабатывать не на одного игрока, а на каждого игрока, который присоединиться к игре.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/20.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('В данном случае у функции нет названия. Это сокращенная запись анонимной функции - функции без названия. Название ей и не нужно, потому что исполнится она по условию срабатывания события добавления нового игрока на сервер. Ни в каком другом случае эта функция срабатывать не будет.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/21.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Допишем строку в теле функции, которая выведет имя подключенного игрока в консоль. '),width=700),
        ft.Container(ft.Text('Что такое ..tostring()? Две точки означают конкатенацию строк, то есть склеивание, соединение строк вместе. Функция tostring() преобразует то, что мы передаем параметром к типу данных - строка.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/22.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('И вот, когда запустим, при добавлении к игре нового пользователя, выведет его ник. Событие сработало.'),width=700),
        ft.Container(ft.Text('Второе событие, которое используется часто - это событие касания. '),width=700),
        ft.Container(ft.Text('Создайте парт, немного поднимите над замелей и заанчёрте его. Так же переименуйте Part в touchPart - типа касаемся парта. Переименовать можно во что угодно, просто мне хочется touchPart'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/23.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('После пишем код:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/24.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Получаем такой результат:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/25.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('В консоль прописывается, не только, что коснулся персонаж, а какая его часть персонажа - голова, или ноги, или еще что.'),width=700),
        ft.Container(ft.Text('ГРУППИРОВКА',text_align='center'),width=700),
        ft.Container(ft.Text('Есть два способа организовать объекты в игре - это модель и папка. Когда вы добавляете объекты из тулбокса, на воркспейсе они вставляются как раз отдельной моделью или папкой.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/26.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Мы можем создавать модель самостоятельно. Для этого наведем на плюсик на воркспейсе и выберем модель. Поместим в модель несколько партов.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/27.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Заметьте, что хотя я добавил 3 отдельных парта, когда к ним поднести курсор мышки, захватываются они все вместе, захватывается модель, а не отдельные парты.'),width=700),
        ft.Container(ft.Text('Если мы будем использовать папку, а не модель, то парты будут выбираться отдельно каждый.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/28.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Поэтому для группировки объектов чаще всего лучше использовать модель, а не папку.'),width=700),
        ft.Container(ft.Text('На этом закончим, а то урок и так затянулся. Мы углубились и разобрали то, что ты делал ранее. Надеюсь, что стало чуть понятнее, как работает код, который мы писали в прошлых уроках.'),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        # ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_16/5.png',width=700),margin=ft.margin.only(left=0)),
...
# 17 урок
less_17 = ft.Container(
    ft.Column(controls=[
        ft.Container(ft.Text('Статистика игроков',text_align='center'),width=700),
        ft.Container(ft.Text('Первое, что нужно понять - это то, что мы можем создавать на воркспейсе не только парты или скрипты, но и отдельные переменные. Например, создадим строку.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_17/1.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('При этом у строки будут свойства - value (значение), Name (имя), к которым можно обращаться. При этом, созданные таким образом переменные, будут глобальными - к ним будет доступ у всех скриптов на чтение и изменение.  Данные переменные создаются, как отдельный объект, как парт, который мы создавали с помощью кода ранее в уроках. Переменные-объекты мы будем использовать в статистике игроков.'),width=700),
        ft.Container(ft.Text('Создадим папку со статистикой лидеров для каждого игрока, который присоединяется к игре. '),width=700),
        ft.Container(ft.Text('Для этого напишем событие добавления нового игрока в игру. (это мы уже изучали в 16 уроке)'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_17/2.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Далее добавляем тело функции'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_17/3.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Запустим и проверим, что папка создается'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_17/4.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Далее создадим монеты.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_17/5.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Запустим и убедимся, что все создается.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_17/6.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Так же у нас создалась таблица лидеров, где указано сколько у нас монет.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_17/7.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Теперь научимся получать монеты. В первую очередь создайте цилиндр и сделайте его в форме монеты, чуть поднимите, заончёрте и переименуйте в coin'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_17/8.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('После создаем скрипт на монете и начинанием писать код'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_17/9.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Если коснулись монеты (событие касания), то находим игрока, который коснулся, и если игрок найден то'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_17/10.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('После этого можно запустить и протестировать, что у нас получилось.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_17/11.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('И всё работает! Монета при касании исчезает и начисляется игроку в таблице лидеров.'),width=700),
        ft.Container(ft.Text('При такой структуре кода, масштабировать игру очень просто. Мы просто копируем монеты и они будут работать по отдельности. Чтобы не мусорить в воркспейсе, создадим папку, назовем ее Coins и перенесем туда монету, размножим и проверим, что все работает.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_17/12.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Отлично! Всё работает. '),width=700),
        ft.Container(ft.Text('ЗАДАНИЕ:'),width=700),
        ft.Container(ft.Text('Сделайте, чтобы монетка не тупо висела в воздухе, а еще и вращалась вокруг своей оси и покачивалась вверх и вниз. Как в нормальных играх.'),width=700),
        ft.Container(ft.Text('Сделай это и покажи преподу. Скажи что ты молодец. '),width=700),
        ft.Container(ft.Text('Подсказка - создай еще два скрипта на монетке. Один будет медленно крутить ее, посмотри, какое свойство отвечает за ориентацию в пространстве. Второй скрипт будет перемещать монетку вверх, потом вниз.'),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
...
# 18 урок
less_18 = ft.Container(
    ft.Column(controls=[
        ft.Container(ft.Text('Создание магазина.',text_align='center'),width=700),
        ft.Container(ft.Text('Помните вы делали игру в контер страйк? В ней самое сложное в плане скриптов было добавление в разные команды. Но контер страйк была неполноценной. Нельзя было выбрать ботов в начале игры , не было очков за убийства и магазина, в котором можно покупать оружие и снарягу. Было бы круто доделать игру?'),width=700),
        ft.Container(ft.Text('Да, но не сегодня) К этому обязательно вернемся.'),width=700),
        ft.Container(ft.Text('Сегодня же придумай игру, где нужно будет собирать в процессе монетки. Добавить через starterGUI кнопку - магазин. Когда нажимаешь, открывается магазин, в котором за собранные монеты можно что-то купить. При покупке этого чего-то, списываются со счета монеты и в инвентаре добавляется предмет.'),width=700),
        ft.Container(ft.Text('Круто же, правда?'),width=700),
        ft.Container(ft.Text('Как делать магазин? Точно так же, как и меню. С этого можете и начать. '),width=700),
        ft.Container(ft.Text('Как сделать логику покупки? Да так же, как и добавления монет при касании монет, только наоборот. При выборе определенной вещи в меню, отнимать от баланса ее стоимость и добавлять в инвентарь игроку предмет.'),width=700),
        ft.Container(ft.Text('Как добавлять предмет в инвентарь игроку?'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_18/1.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Пример кода, выдачи катаны игроку на старте. Добавляем предмет из тулбокса в серверное хранилище, когда нам нужно, например, когда игрок покупает в магазине предмет - копируем предмет из хранилища и выдаем игроку в инвентарь. Все очень просто!'),width=700),
        ft.Container(ft.Text('Самое главное - придумайте игру, завязанную на данной механике. Это может быть лабиринт, откуда нужно выбраться и вы за монеты покупаете пружины, или старый добрый паркур (хотя он уже поднадоел) Или может быть шутер в городе. Нужно бежать вперед, убивать зомбаков и собирать монеты в процессе. За собранные монеты в магазине покупать новое оружие. Короче думайте. Только эти примеры я придумал. Их использовать нельзя. Придумайте что-то свое. Главное, что это уже полноценная механика, на которой можно сделать интересную игру. Вперед!'),width=700),
        ft.Container(ft.Text('Когда придумаешь принцип игры, расскажи преподу. Он подскажет, что делать дальше, если не понимаешь.'),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        # ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_18/5.png',width=700),margin=ft.margin.only(left=0)),
    ])
)

# 19 урок
less_19 = ft.Container(
    ft.Column(controls=[
        ft.Container(ft.Text('Полоса препятствий.',text_align='center'),width=700),
        ft.Container(ft.Text('Вы заметили, что постепенно в уроках становиться больше самостоятельной работы? Вначале было много самостоятельной работы в плане дизайна и построек, теперь все больше кода приходится придумывать и писать самому. Это не просто так, а для того, чтобы ты действительно чему-то научился. В мире куча курсов, где ты слушаешь препода, повторяешь за ним, получаешь грамотку, что ты освоил курс, но на выходе совершенно ничего не можешь повторить или написать самостоятельно. Я не хочу, чтобы этот курс был одним из таких, поэтому и даю все больше самостоятельной работы. Тем более, что ты уже действительно прошел много уроков и должен хоть что-то понимать в разработке игр в роблокс. Так вот видео того, какую игру нужно сделать:'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_19/1.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('У тебя будет полоса препятствий, которую нужно пройти. Она будет разделена на 5 уровней. Логика такая - ты нажимаешь на красную кнопку, стена во второй уровень становится прозрачной на како-то количество секунд (запринтить в оутпуте), ты бежишь и собираешь в процессе шары, когда касаешься шара , он тухнет и тебе выдается рандомное количесво очков от 1 до 5. Если не успеваешь добежать за зеленую стену в установленное время, она снова становится непроницаемой и нужно заново нажимать на кнопку и так далее.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_19/2.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Я сделал достаточно простую графику. Ты же придумай что-то поинтереснее. Стилизуй это в готовую игру. '),width=700),
        ft.Container(ft.Text('У тебя действительно есть всё, чтобы сделать эту игру. Тут нет совсем ничего нового. Мы все проходили раньше, в прошлых уроках. Не знаешь с чего начать? Начни, как и всегда с графики. После реализуй систему красных кнопок и прозрачных дверей по таймеру. И в самом конце сделай шары и добавление очков.'),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        # ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_19/5.png',width=700),margin=ft.margin.only(left=0)),
    ])
)

# 20 урок
less_20 = ft.Container(
    ft.Column(controls=[
        ft.Container(ft.Text('Потеем над графикой',text_align='center'),width=700),
        ft.Container(ft.Text('Я думаю, что вы могли немного устать от обилия скриптов в прошлых нескольких уроках, поэтому следующие 3-4 урока посвятим графическому интерфейсу GUI. Сделаем игру красивой и функциональной. Сейчас максимум, что ты можешь делать с GUI, это делать меню, или магазин на основе меню. Будем развивать эту историю дальше. Мы часто выводили время в консоль и это дико неудобно. Ведь у игроков не будет консоли, когда они будут играть в твои игры. Давай добавим отображение чего угодно на экране. Делается это действительно просто. '),width=700),
        ft.Container(ft.Text('Создайте скрин гуи в стартер гуи, в нем создайте textlabel. И немного стилизуйте его, пример ниже'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_20/1.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('После создаем на textlabel скрипт и пишем следующее'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_20/2.png',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('Запустим и проверим, что будет.'),width=700),
        ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_20/3.gif',width=700),margin=ft.margin.only(left=0)),
        ft.Container(ft.Text('После всего, что ты пережил в прошлых уроках, это выглядит, как глоток свежего воздуха. Так вот теперь ты можешь выводить информацию на экран. Любую, какую захочешь. Твои игры станут еще круче.'),width=700),
        ft.Container(ft.Text('На этом в уроке все. Даже доп задания не будет. Просто мини урок. '),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        ft.Container(ft.Text(''),width=700),
        # ft.Container(ft.Image(src=f'{path_img}\\img_roblox/les_20/5.png',width=700),margin=ft.margin.only(left=0)),
    ])
)
